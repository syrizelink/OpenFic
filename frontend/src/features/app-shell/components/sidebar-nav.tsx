...
function SidebarNavItem({ item, isExpanded }: { item: AppSidebarNavItem; isExpanded: boolean }) {
  ...
  const content = (
    ...
    >
      ...
      >
        <Icon size={SIDEBAR_ICON_SIZE} color="currentColor" className="app-sidebar-nav-item__icon" />
      </Box>
      <motion.div
        initial={false}
        animate={{
      ...
      >
        <Text size="2" weight={item.active ? "bold" : "medium"}>
          {item.label}
        </Text>
      </motion.div>
    </MotionFlex>
...
export function SidebarNav({ items, isExpanded }: SidebarNavProps) {
  return (
    <Flex direction="column" align="center" gap="1" width="100%">
      {items.map((item) => (
        <SidebarNavItem key={item.href} item={item} isExpanded={isExpanded} />
      ))}
    </Flex>
  );
}
