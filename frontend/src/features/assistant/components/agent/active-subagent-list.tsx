...
export function ActiveSubagentList({
  items,
  title,
  onOpen,
}: ActiveSubagentListProps) {
  ...
  return (
    <Box className="active-subagent-panel" aria-label={title}>
      <AnimatePresence initial={false}>
        {!collapsed ? (
          ...
          >
            {items.map((item) => (
              <li key={item.childRunId} className="active-subagent-panel-list-item">
                <button
                  type="button"
                  onClick={() => onOpen(item)}
                ...
                >
                    <span className="active-subagent-row-main">
                      <span className="active-subagent-row-indicator" aria-hidden="true" />
                      <Text as="span" size="1" weight="medium" className="active-subagent-row-agent">
                        {formatSubagentDisplayLabel(item.agentKey, item.agentNumber)}
                      </Text>
                    </span>
                  <span className="active-subagent-row-meta">
                    {item.queuedMessages > 0 ? (
                      <Text as="span" size="1" className="active-subagent-row-queue">
                        +{item.queuedMessages}
                      </Text>
                    ) : null}
                    <Text as="span" size="1" className="active-subagent-row-status">
      ...
      >
        <span className="active-subagent-panel-title-group">
          <span className="active-subagent-panel-icon-wrap" aria-hidden="true">
            <Bot size={14} />
          </span>
          <Text as="span" size="1" className="active-subagent-panel-title">
