...
import { getChapterPayload, getToolResultMessage } from "../shared/tool-message-utils";
...
export function ChapterToolMessage({ message }: ChapterToolMessageProps) {
  ...
  const isMutationTool =
    message.toolName === "write_chapter" || message.toolName === "edit_chapter";
  ...
  if (diffPreview) {
    ...
    const isTitleOnlyChange =
      !diffSection && diffPreview.sections.some((section) => section.type === "title");
      ...
      <ToolBody>
        <Box className="agent-chapter-diff-card">
          ...
          >
            ...
            >
              <Text className="agent-chapter-diff-card-title">{chapterName}</Text>
              <Flex
                align="center"
                gap="1"
          ...
          <Box className="agent-chapter-diff-card-body">
            <Box className="agent-chapter-diff-scroll">
              <div className="agent-chapter-diff-lines">
                {diffSection?.lines.length ? (
                  diffSection.lines.map((line, index) => (
                    <div
                  ...
                  <div className="agent-chapter-diff-empty">
                    {isTitleOnlyChange
                      ? i18n.t("assistant.tools.titleOnlyChange")
      ...
      <ToolTextBlock
        ...
        value={
          typeof chapter.word_count === "number"
            ? i18n.t("assistant.tools.wordCountWithUnit", { count: chapter.word_count })
            : undefined
        }
      />
      {chapter.content ? (
