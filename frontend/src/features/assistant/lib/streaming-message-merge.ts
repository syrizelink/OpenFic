...
function isStreamingDeltaMessage(message: AgentMessage): boolean {
  return Boolean(
    message.payload?.is_delta
    && ["text", "reasoning", "tool"].includes(message.type)
  );
}

export function getPersistedReasoningDurationMs(message: Pick<AgentMessage, "thinkingDurationMs" | "payload">): number {
...
export function getReasoningDurationMs(
  message: Pick<AgentMessage, "timestamp" | "thinkingDurationMs" | "payload" | "isStreaming" | "status">,
  now = Date.now()
): number {
...
export function mergeStreamingMessage(previous: AgentMessage, message: AgentMessage): AgentMessage {
  ...
  const nextContent = isDelta && ["text", "reasoning"].includes(message.type)
  ...
  return {
    ...
    ...message,
    id: previous.id,
    payload: isDelta ? { ...(previous.payload ?? {}), ...(message.payload ?? {}) } : message.payload,
    content: nextContent,
    ...
    toolArgsText: message.status === "running" ? message.toolArgsText ?? previous.toolArgsText : undefined,
    partialToolArgs: message.status === "running" ? message.partialToolArgs ?? previous.partialToolArgs : undefined,
