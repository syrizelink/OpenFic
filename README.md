...
- 💾**默认本地持久化**：默认使用 SQLite 将项目数据保存在本地，自托管部署可选择 PostgreSQL
...
默认使用 SQLite，无需数据库服务。部署时可选 PostgreSQL；现有数据不会自动迁移。

- [首次部署与数据库选择](docs/database-deployment.zh-CN.md)
- [SQLite → PostgreSQL 迁移与回退](docs/database-migration.zh-CN.md)
