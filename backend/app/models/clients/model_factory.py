...
def create_chat_model(config: ModelConfig) -> BaseChatModel:
    ...
    if provider == "deepseek":
        ...
        if (
            config.deepseek_thinking_type == "enabled"
            and config.deepseek_reasoning_effort
        ):
            deepseek_kwargs["reasoning_effort"] = config.deepseek_reasoning_effort
        if config.deepseek_thinking_type:
    ...
    return ChatOpenAI(**_compact_kwargs(
        model=config.model_id,
        api_key=config.api_key,
        base_url=config.base_url or None,
        temperature=config.temperature,
